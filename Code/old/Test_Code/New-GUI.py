...
class MainWindow(QMainWindow):
    ...
    def addDynamicTab(self, script):
        '''Dynamically adds a tab for a script.'''
        try:
            module = importlib.import_module(script["path"])
            gui_entry = getattr(module, script["gui_entry"])()
            if isinstance(gui_entry, QWidget):
                # Create a new QWidget for the tab and set its layout to contain the script's GUI component
                tab = QWidget()
                layout = QVBoxLayout(tab)
                layout.addWidget(gui_entry)  # Add the script's GUI component to the tab's layout
